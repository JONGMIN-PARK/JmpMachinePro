// ThemeCheck.cpp : implementation file
//
...
// 테마 관련 변수 초기화
void CThemeCheck::InitThemeVariable()
{
	...
	m_stThemeInfo.crMark = RGB(8,3,69);							// 마크 컬러
	m_stThemeInfo.nMarkSize = 12;								// 마크 크기
...
BEGIN_MESSAGE_MAP(CThemeCheck, CButton)
	//{{AFX_MSG_MAP(CThemeCheck)
	ON_WM_SIZE()
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_DESTROY()
...
/////////////////////////////////////////////////////////////////////////////
// CThemeCheck message handlers

// 클래스 생성
BOOL CThemeCheck::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
	// 체크박스 속성을 설정
...
// 클래스 생성시 실행되는 함수
int CThemeCheck::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CThemeCheck::OnSize(UINT nType, int cx, int cy) 
{
...
void CThemeCheck::PreSubclassWindow() 
{
	// 기본 윈도우 지원 스킨을 방지
	ModifyStyle(0, BS_OWNERDRAW | BS_AUTOCHECKBOX);//BS_PUSHBUTTON);
	
	CButton::PreSubclassWindow();
}

void CThemeCheck::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
...
// 컨트롤 전체를 그리는 함수
void CThemeCheck::DrawControlTotal(CPaintDC *pdc)
{
	// 화면 영역좌표 구하기
	CRect rScreen;
	this->GetClientRect(&rScreen);
...
// Caption 문자열을 화면에 표시한다
void CThemeCheck::DrawCaptionText(CPaintDC *pdc, CRect rScreen)
{
	CRect rFont, rShadow;
	int nBtnState=0;
	BOOL bCheck=FALSE;
	...
	// PushLike 상태 이면서 Check 상태이고 일반 상태일때
	if( m_stThemeInfo.bPushLike && bCheck && m_stThemeInfo.nBtnState == ST_NORMAL )
		// 버튼 상태를 Pressed로 설정
...
// Caption 문자열 설정
void CThemeCheck::SetCaptionText(CString strCaption)
{
	// 전역변수에 저장
...
// Caption 문자열을 설정
void CThemeCheck::SetWindowText(CString strText)
{
	SetCaptionText(strText);
}

// Caption 영역좌표 설정
void CThemeCheck::SetCaptionArea(CRect rCaption)
{
	// 전역변수에 저장
...
// Caption 정렬 방식 설정
void CThemeCheck::SetAlignFormat(UINT nAlignFormat)
{
	// 전역변수에 저장
...
// 폰트 크기 설정
void CThemeCheck::SetFontSize(int nFontSize)
{
	m_stThemeInfo.nFontSize = nFontSize;
}

// 폰트 두께 설정
void CThemeCheck::SetFontBold(BOOL bBold)
{
	m_stThemeInfo.bFontBold = bBold;
}

// 배경 모서리 둥근 정도 설정
void CThemeCheck::SetEdgeRoundValue(int nRoundValue)
{
	m_stThemeInfo.nEdgeRoundValue = nRoundValue;
}

// 외곽선 두께 설정
void CThemeCheck::SetOutlineThick(int nThick)
{
	m_stThemeInfo.nOutlineThick = nThick;
}

// 외곽선(Left-Top) 컬러 설정
void CThemeCheck::SetOutlineLTColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineLT = crOutline;
}

// 외곽선(Right-Bottom) 컬러 설정
void CThemeCheck::SetOutlineRBColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineRB = crOutline;
}

// 외곽선 컬러 설정
void CThemeCheck::SetOutlineColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineLT = crOutline;
	m_stThemeInfo.crOutlineRB = crOutline;
}
...
// 배경 그리기 방식 설정
void CThemeCheck::SetBackDrawType(int nDrawType)
{
	m_stThemeInfo.nBackDrawType = nDrawType;
}

// 배경 이미지 채우기 방식 설정
void CThemeCheck::SetBackImageFillType(int nImageFillType)
{
	m_stThemeInfo.nBackImageFillType = nImageFillType;
}

// 배경 이미지 정렬방식 설정
void CThemeCheck::SetBackImageAlignFormat(UINT nAlignFormat)
{
	m_stThemeInfo.nBackImageAlignFormat = nAlignFormat;
}

// 툴팁을 생성한다
void CThemeCheck::MakeToolTip(CString strToolTip)
{
	// 현 컨트롤 ID를 구한다
...
// 툴팁 텍스트 변경
void CThemeCheck::ChangeToolTip(CString strToolTip)
{
	// 툴팁 텍스트 변경
...
// 툴팁 삭제
void CThemeCheck::DeleteToolTip()
{
	if( !m_ToolTip.GetSafeHwnd() )
		return;
...
// 툴팁을 위한 이벤트를 받는 함수
LRESULT CThemeCheck::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message)
	{
		case WM_LBUTTONDOWN:
...
// 폰트 컬러 설정
void CThemeCheck::SetFontAndShadowColor(COLORREF crFont, COLORREF crShadow)
{
	// State Normal
...
// 폰트 특성 설정
void CThemeCheck::SetFont(int nFontSize, BOOL bFontBold, COLORREF crFont, COLORREF crShadow, UINT nAlignFormat)
{
	// 폰트 특성을 전역변수에 저장
	m_stThemeInfo.nFontSize = nFontSize;
...
// 배경 컬러 설정
void CThemeCheck::SetBackColor(COLORREF crBackNormal, COLORREF crBackHover, COLORREF crBackPressed, COLORREF crBackDisabled)
{
	m_stThemeInfo.nBackDrawType = dfBACK_DRAW_TYPE_COLOR;		// 배경 그리기 방식
...
void CThemeCheck::CheckHover(CPoint point)
{
...
// 마우스 Hover시 실행되는 함수
LRESULT CThemeCheck::OnMouseHover(WPARAM wParam, LPARAM lParam)
{
	// 마우스 Hover 상태로 설정
...
// 마우스 Leave시 실행되는 함수
LRESULT CThemeCheck::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	// 마우스 Hover 상태로 설정
	if( m_stThemeInfo.bHover )
...
void CThemeCheck::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// 마우스 클릭 상태 설정
...
// 마우스 이동시 실행되는 함수
void CThemeCheck::OnMouseMove(UINT nFlags, CPoint point) 
{
	// 마우스 클릭 상태라면
...
// 배경 이미지 블럭 개수 설정
void CThemeCheck::SetBackImageBlockCount(int nBlock)
{
...
CRect CThemeCheck::GetImagePartByState(int nState)
{
...
// 폰트 컬러 설정
void CThemeCheck::SetFontColor(COLORREF crFontNormal, COLORREF crFontHover, COLORREF crFontPressed, COLORREF crFontDisabled)
{
	// State Normal
...
// 그림자 컬러 설정
void CThemeCheck::SetShadowColor(COLORREF crShadowNormal, COLORREF crShadowHover, COLORREF crShadowPressed, COLORREF crShadowDisabled)
{
	// State Normal
...
// 폰트 Hover 컬러 설정
void CThemeCheck::SetFontHoverColor(COLORREF crFontHover)
{
	m_stThemeInfo.crFont[ST_HOVER] = crFontHover;
}

// 폰트 Pressed 컬러 설정
void CThemeCheck::SetFontPressedColor(COLORREF crFontPressed)
{
	m_stThemeInfo.crFont[ST_PRESSED] = crFontPressed;
}

// 폰트 Disabled 컬러 설정
void CThemeCheck::SetFontDisabledColor(COLORREF crFontDisabled)
{
	m_stThemeInfo.crFont[ST_DISABLED] = crFontDisabled;
}

// 그림자 Hover 컬러 설정
void CThemeCheck::SetShadowHoverColor(COLORREF crShadowHover)
{
	m_stThemeInfo.crShadow[ST_HOVER] = crShadowHover;
}

// 그림자 Pressed 컬러 설정
void CThemeCheck::SetShadowPressedColor(COLORREF crShadowPressed)
{
	m_stThemeInfo.crShadow[ST_PRESSED] = crShadowPressed;
}

// 그림자 Disabled 컬러 설정
void CThemeCheck::SetShadowDisabledColor(COLORREF crShadowDisabled)
{
	m_stThemeInfo.crShadow[ST_DISABLED] = crShadowDisabled;
}

// PushLike 여부 설정
void CThemeCheck::SetPushLike(BOOL bPushLike)
{
	m_stThemeInfo.bPushLike = bPushLike;
}

// 마크 그리기 방식 설정
void CThemeCheck::SetMarkDrawType(int nDrawType)
{
...
// 마크 컬러 설정
void CThemeCheck::SetMarkColor(COLORREF crMark)
{
	m_stThemeInfo.crMark = crMark;
}

// 배경 이미지를 여는 함수(파일)
BOOL CThemeCheck::SetBackImage(CString strFileName)
{
	CString strFilePath;
	BOOL bBmpLoad=FALSE;
...
BOOL CThemeCheck::SetBackImage(UINT nImageID)
{
	BOOL bBmpLoad=FALSE;
...
// 마크 이미지를 여는 함수(파일)
BOOL CThemeCheck::SetMarkImage(CString strFileName)
{
	CString strFilePath;
	BOOL bBmpLoad=FALSE;
...
BOOL CThemeCheck::SetMarkImage(UINT nImageID)
{
	BOOL bBmpLoad=FALSE;
...
void CThemeCheck::OnLButtonUp(UINT nFlags, CPoint point) 
{
	// 마우스 클릭 상태 해제
	m_stThemeInfo.bPressed = FALSE;
...
// 체크박스 컨트롤 마우스 Click 시에 처리하는 함수
void CThemeCheck::ProcessClickCtrl()
{
	BOOL bCheck=FALSE;
...
// 마크를 컬러로 그린다
void CThemeCheck::DrawMarkByColor(CPaintDC *pdc, CRect rScreen)
{
	CRect rMark;
	CPoint po1, po2;
	...
	// 테두리 사각형을 그린다
	gf_DrawRectLine(rMark, m_stThemeInfo.crOutlineLT, m_stThemeInfo.crOutlineLT, m_stThemeInfo.crOutlineRB, m_stThemeInfo.crOutlineRB, pdc);
	...
	if( m_stThemeInfo.bCheck )
	{
		...
		// 라인(선)을 그리는 함수
		gf_DrawLine(po1, po2, m_stThemeInfo.crMark, 2, pdc);
...
// 배경을 지울때 실행되는 함수
BOOL CThemeCheck::OnEraseBkgnd(CDC* pDC) 
{
	// 화면 번쩍거림을 방지
	return true;
...
// 컨트롤 위치좌표를 부모 윈도우에 대한 상대좌표로 반환
CRect CThemeCheck::GetCtrlRectByParent()
{
	CRect rCtrl, rParent;
...
// 비트맵 이미지로 마크를 그린다
void CThemeCheck::DrawMarkByImage(CPaintDC *pdc, CBitmap *pBitmap, CRect rScreen)
{
	CRect rView, rMark;
...
// Check 상태 설정
void CThemeCheck::SetCheck(BOOL bCheck)
{
	m_stThemeInfo.bCheck = bCheck;

	// 배경을 새로 그린다
	Invalidate(FALSE);
}
// Check 상태 반환
BOOL CThemeCheck::GetCheck()
{
	return m_stThemeInfo.bCheck;
}

// 컬러로 배경을 그린다
void CThemeCheck::DrawBackByColor(CPaintDC *pdc, COLORREF crBack, CRect rScreen)
{
...
// 비트맵 이미지로 배경을 그린다
void CThemeCheck::DrawBackByImage(CPaintDC *pdc, CBitmap *pBitmap, CRect rScreen)
{
	int nBtnState=0;
	BOOL bCheck=FALSE;
...
// 툴팁 문자열 설정
void CThemeCheck::SetToolTip(CString strText)
{
	// 툴팁이 이미 생성되어 있다면
	if( m_ToolTip.GetSafeHwnd() )
		// 툴팁 문자열 변경
...
// 윈도우 생성 전에 실행되는 함수
BOOL CThemeCheck::PreCreateWindow(CREATESTRUCT& cs) 
{
	// 'PushLike' 속성이 존재한다면
