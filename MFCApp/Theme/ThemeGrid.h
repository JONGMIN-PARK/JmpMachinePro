/********************************************************************/
/*																	*/
/*		FILE NAME		:	ThemeGrid.h								*/
/*		DISCRIPTION		:	Theme Grid Ctrl Skin Control Class		*/
/*																	*/
...
#if !defined(AFX_THEMEGRID_H__4C74D66C_4E5D_4076_93D2_8F67032AAF45__INCLUDED_)
#define AFX_THEMEGRID_H__4C74D66C_4E5D_4076_93D2_8F67032AAF45__INCLUDED_

#if _MSC_VER > 1000
#pragma once
#endif // _MSC_VER > 1000
// ThemeGrid.h : header file
//
...
// Hit GridCtrlEx values (not yet implemented)
...
// add SKC ---------------------------------------------------
#define GVN_BUTTONDOWN			WM_LBUTTONDOWN
#define GVN_DOUBLECLICKED		WM_LBUTTONDBLCLK
#define GVN_RBUTTONDOWN			WM_RBUTTONDOWN
// end SKC ---------------------------------------------------
...
// add SKC ----------------------------------
class CColInfoObject : public CObject
{
public:
	CColInfoObject() { nArray = 0; };
	~CColInfoObject() { if (nArray > 0) sArray.RemoveAll(); };
	int nArray;
	CStringArray sArray;
};
class CColGroupObject : public CObject
{
public:
	CColGroupObject() { nArray = 0; };
	~CColGroupObject() { if (nArray > 0) iArray.RemoveAll(); };
	int nArray;
	CUIntArray iArray;
	CString csText;
};
...
class CThemeGrid : public CWnd
{
    DECLARE_DYNCREATE(CThemeGrid)
...
public:
	...
	static struct ST_THEME_GRID_INFO
	{
		CString		strIniCtrlName;			// INI 파일 조회용 컨트롤명
		CRect		rInitPos;				// 초기 위치 정보
		CPoint		poCell;					// Cell 위치
	};

    CThemeGrid(int nRows = 0, int nCols = 0, int nFixedRows = 0, int nFixedCols = 0);

    BOOL Create(DWORD dwStyle, const RECT& rect, CWnd* parent, UINT nID);
	BOOL Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle=WS_TABSTOP|WS_CHILD|WS_VISIBLE|WS_BORDER);
    BOOL SubclassWindow(HWND hWnd);

public:
	CObList	m_ColInfoList;
	CObList	m_ColGroupList;
	CUIntArray	m_arColumnCheckbox;
	int			m_nFontSize;
	CFont		m_ftText;
    ...
    // General attributes
    COLORREF    m_crTextColour;
	COLORREF    m_crTextBkColour;
	COLORREF    m_crBkColour;   // Grid colours
    COLORREF    m_crFixedTextColour;
	COLORREF    m_crFixedBkColour;
	COLORREF    m_crGridColour;
    COLORREF    m_crWindowText;
	COLORREF    m_crWindowColour;
	COLORREF    m_cr3DFace;     // System colours
    COLORREF    m_crShadow;
	COLORREF	m_crFixedFrameLT;
	COLORREF    m_crFixedFrameRB;
...
/*#ifndef GRIDCONTROL_NO_DRAGDROP
    ...
    int         m_bAscending;
    int         m_SortColumn;
	ST_THEME_GRID_INFO		m_stThemeInfo;

	BOOL		m_bBaseCtrlList;		// Is Base Control ListCtrl?
...
// add SKC ---------------------------
	void CreateInPlaceComboControl(CRect& rect, DWORD dwStyle, UINT nID,
								   int nRow, int nColumn, 
								   CStringArray& Items, CString sInitText, 
								   UINT nFirstChar);

	void	SetColComboBox(int nCol, wchar_t *pArray[], int NumArray);
	BOOL	IsColComboBox(int nCol, CStringArray &sArray);
	void	SetColGroup(int *pGroupArray, int NumArray, LPCTSTR szGroupName);
	BOOL	IsColGroup(int nCol, int &Id, int &FirstCol, int &LastCol, CString &csText);
	void	ExpandColsToFit(int nColumn);
	void	ClearSelectedRange();

	BOOL	(CWnd::*m_fpExInfoFun)(int nRow, int nCol, COLORREF &BkColor);
	void	SetExInfoCallback(BOOL (CWnd::*fpExInfoFun)(int nRow, int nCol, COLORREF &BkColor));
...
// end SKC ---------------------------
    ...
    void     SetNormalFontColor(COLORREF clr)     { m_crTextColour = clr;             }
    COLORREF GetNormalFontColor() const           { return m_crTextColour;            }

    void     SetNormalBackColor(COLORREF clr)     { m_crTextBkColour = clr;           }
    COLORREF GetNormalBackColor() const           { return m_crTextBkColour;          }

    void     SetBackColor(COLORREF clr)             { m_crBkColour = clr;               }
    COLORREF GetBackColor() const                   { return m_crBkColour;              }
    void     SetFixedFontColor(COLORREF clr)      { m_crFixedTextColour = clr;        }
    COLORREF GetFixedFontColor() const            { return m_crFixedTextColour;       }
    void     SetFixedBackColor(COLORREF clr)      { m_crFixedBkColour = clr;          }
    COLORREF GetFixedBackColor() const            { return m_crFixedBkColour;         } 
    void     SetFrameColor(COLORREF clr)           { m_crGridColour = clr;             }
    COLORREF GetFrameColor() const                 { return m_crGridColour;            }
	void     SetFixedFrameLTColor(COLORREF clr)   { m_crFixedFrameLT = clr;            }
    COLORREF GetFixedFrameLTColor() const         { return m_crFixedFrameLT;           }
	void     SetFixedFrameRBColor(COLORREF clr)   { m_crFixedFrameRB = clr;            }
    COLORREF GetFixedFrameRBColor() const         { return m_crFixedFrameRB;           }
	void     SetFixedFrameColor(COLORREF clr)     { m_crFixedFrameRB = clr;	m_crFixedFrameRB = clr; }
	...
	void SetEditDblClk(BOOL bEditDblClk = TRUE)   { m_bEditDblClk = bEditDblClk;      }
    ...
    void SetColResize(BOOL bResize = TRUE)		  { m_bAllowColumnResize = bResize;   }
    BOOL GetColResize() const                     { return m_bAllowColumnResize;      }
    void SetHeadSort(BOOL bSortOnClick = TRUE)  { m_bSortOnClick = bSortOnClick;    }
    BOOL GetHeadSort() const                    { return m_bSortOnClick;            }
...
// Implementation
public:
	void SetBaseCtrlList(BOOL bFlag);
	BOOL SetCols(int nCols);
	BOOL SetRows(int nRows);
	void SetData(LPARAM lParam);
	void SetText(CString strText);
	void SetCol(long nNewValue);
	void SetRow(long nNewValue);
	BOOL SetFixedRows(int nFixedRows);
	BOOL SetFixedCols(int nFixedCols);
	...
	BOOL PorcessAfterCreate();
	void DrawCheckBoxByColor(CDC *pDC, CRect RectCheck, BOOL bCheck);
	BOOL SetImageList(CString strFileName, int cx, int nGrow, COLORREF crMask);
	BOOL SetCheckBoxImage(CString strFileName);
	BOOL SetCheckBoxImage(UINT nImageID);
	void SetAlignFormat(UINT nAlignFormat);
	BOOL GetFocusCell(int &nRow, int &nCol);
	void SetColComboBox(int nCol, CString strArray);
	void SetFontSize(int nFontSize);
	void CreateTextFont(int nFontSize);
	void SetCellWidth(int nWidth);
	void SetCellHeight(int nHeight);
	BOOL SetImageList(UINT nBitmapID, int cx, int nGrow, COLORREF crMask);
	BOOL IsColCheckBox(UINT nCol);
	void SetColCheckBox(UINT nCol);
	virtual ~CThemeGrid();
...
protected:
    void SelectAllCells();
    void SelectCols(CCellID currentCell);
    ...
    afx_msg BOOL OnSetCursor(CWnd* pWnd, UINT nHitGridCtrlEx, UINT message);
...
#endif // !defined(AFX_THEMEGRID_H__4C74D66C_4E5D_4076_93D2_8F67032AAF45__INCLUDED_)
