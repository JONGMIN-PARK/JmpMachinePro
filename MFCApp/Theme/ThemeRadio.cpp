// ThemeRadio.cpp : implementation file
//
...
// 테마 관련 변수 초기화
void CThemeRadio::InitThemeVariable()
{
	...
	// 마크 그리기 방식이 이미지라면 기존 이미지를 삭제
	if( m_stThemeInfo.nMarkDrawType == dfMARK_DRAW_TYPE_IMAGE )
		m_stThemeInfo.bmpMark.DeleteObject();
	...
	m_stThemeInfo.crFont[ST_DISABLED] = RGB(255,255,255);		// 폰트 컬러(Disabled)
	...
	m_stThemeInfo.crBackground[ST_NORMAL] = RGB(214,225,227);	// 배경 컬러
	m_stThemeInfo.crBackground[ST_HOVER] = RGB(214,225,227);	// 배경 컬러
	m_stThemeInfo.crBackground[ST_PRESSED] = RGB(214,225,227);	// 배경 컬러
	...
	m_stThemeInfo.nOutlineThick = 0;							// 테두리 두께
	...
	m_stThemeInfo.bPushLike = FALSE;							// PushLike 여부
	m_stThemeInfo.nMarkDrawType = dfMARK_DRAW_TYPE_COLOR;		// 마크 그리기 방식
	m_stThemeInfo.crMark = RGB(20,60,110);						// 마크 컬러
	m_stThemeInfo.nMarkSize = 13;								// 마크 크기
...
BEGIN_MESSAGE_MAP(CThemeRadio, CButton)
	//{{AFX_MSG_MAP(CThemeRadio)
...
/////////////////////////////////////////////////////////////////////////////
// CThemeRadio message handlers

// 클래스 생성
BOOL CThemeRadio::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
	// 라디오 버튼 속성을 설정
...
// 클래스 생성시 실행되는 함수
int CThemeRadio::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CThemeRadio::OnSize(UINT nType, int cx, int cy) 
{
...
// 컨트롤 전체를 그리는 함수
void CThemeRadio::DrawControlTotal(CPaintDC *pdc)
{
	// 화면 영역좌표 구하기
	CRect rScreen;
	this->GetClientRect(&rScreen);
	...
	// 마크 그리기 방식에 따라서
	switch( m_stThemeInfo.nMarkDrawType )
	{
	// 컬러로 마크를 그린다면
	case dfMARK_DRAW_TYPE_COLOR :
		// 마크를 컬러로 그린다
		DrawMarkByColor(pdc, rScreen);
		break;
	// 이미지로 마크를 그린다면
	case dfMARK_DRAW_TYPE_IMAGE :
		// 비트맵 이미지로 마크를 그린다
		DrawMarkByImage(pdc, &m_stThemeInfo.bmpMark, rScreen);
		break;
	default :
		break;
	}
...
// 컬러로 배경을 그린다
void CThemeRadio::DrawBackByColor(CPaintDC *pdc, COLORREF crBack, CRect rScreen)
{
	COLORREF crOutlineLT, crOutlineRB;
	BOOL bCheck=FALSE;

	// 현재 Check 상태를 구한다
	bCheck = this->GetCheck();
	...
	{
		//crBack = m_stThemeInfo.crBackground[m_stThemeInfo.nBtnState];
		// 외곽선 컬러
		...
		// PushLike 상태 이면서 Check 상태 일때
		else if( m_stThemeInfo.bPushLike && bCheck )
		{
			// 외곽선 컬러
			crOutlineLT = m_stThemeInfo.crOutlineRB;
			crOutlineRB = m_stThemeInfo.crOutlineLT;
			...
			// 배경 컬러
			crBack = m_stThemeInfo.crBackground[ST_PRESSED];
		}
		
		// 테두리를 가지는 사각형을 그리는 함수
		gf_DrawRectWithOutline(pdc, crBack, crOutlineLT, crOutlineRB, m_stThemeInfo.nOutlineThick, rScreen);
	}
}

// Caption 문자열을 화면에 표시한다
void CThemeRadio::DrawCaptionText(CPaintDC *pdc, CRect rScreen)
{
	CRect rFont, rShadow;
	int nBtnState=0;
	BOOL bCheck=FALSE;
	...
	// 현재 버튼 상태
	nBtnState = m_stThemeInfo.nBtnState;
	// 현재 Check 상태를 구한다
	bCheck = this->GetCheck();

	// PushLike 상태 이면서 Check 상태 일때
	if( m_stThemeInfo.bPushLike && bCheck )
		// 버튼 상태를 Pressed로 설정
		nBtnState = ST_PRESSED;

	// 마크를 그린다면
	if( !m_stThemeInfo.bPushLike )
	{
		CRect rMark;
		// 마크가 표시될 영역좌표
		rMark = GetMarkArea();
		...
		// 화면영역에서 마크 영역을 제외
		rScreen.left += rMark.right + 5;
		// 배경을 그린다면 오른쪽 여백을 약간 둔다
	...
	if( m_stThemeInfo.crShadow[nBtnState] != dfCOLOR_TRANSPARENT )
		// Caption 그림자를 화면에 표시
		gf_DrawText(pdc, m_stThemeInfo.strCaption, m_stThemeInfo.nFontSize, 
			m_stThemeInfo.crShadow[nBtnState], rShadow, 
			m_stThemeInfo.nAlignFormat|DT_SINGLELINE, m_stThemeInfo.bFontBold);

	// Caption 텍스트를 화면에 표시
	gf_DrawText(pdc, m_stThemeInfo.strCaption, m_stThemeInfo.nFontSize, 
			m_stThemeInfo.crFont[nBtnState], rFont, 
			m_stThemeInfo.nAlignFormat|DT_SINGLELINE, m_stThemeInfo.bFontBold);
}

// Caption 문자열 설정
void CThemeRadio::SetCaptionText(CString strCaption)
{
	// 전역변수에 저장
...
// Caption 문자열을 설정
void CThemeRadio::SetWindowText(CString strText)
{
	SetCaptionText(strText);
}

// Caption 영역좌표 설정
void CThemeRadio::SetCaptionArea(CRect rCaption)
{
	// 전역변수에 저장
...
// Caption 정렬 방식 설정
void CThemeRadio::SetAlignFormat(UINT nAlignFormat)
{
	// 전역변수에 저장
...
//  폰트 크기 설정
void CThemeRadio::SetFontSize(int nFontSize)
{
	m_stThemeInfo.nFontSize = nFontSize;
}

// 폰트 두께 설정
void CThemeRadio::SetFontBold(BOOL bBold)
{
	m_stThemeInfo.bFontBold = bBold;
}

// 배경 모서리 둥근 정도 설정
void CThemeRadio::SetEdgeRoundValue(int nRoundValue)
{
	m_stThemeInfo.nEdgeRoundValue = nRoundValue;
}

// 외곽선 두께 설정
void CThemeRadio::SetOutlineThick(int nThick)
{
	m_stThemeInfo.nOutlineThick = nThick;
}

// 외곽선(Left-Top) 컬러 설정
void CThemeRadio::SetOutlineLTColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineLT = crOutline;
}

// 외곽선(Right-Bottom) 컬러 설정
void CThemeRadio::SetOutlineRBColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineRB = crOutline;
}

// 외곽선 컬러 설정
void CThemeRadio::SetOutlineColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineLT = crOutline;
	m_stThemeInfo.crOutlineRB = crOutline;
}
...
// 배경 그리기 방식 설정
void CThemeRadio::SetBackDrawType(int nDrawType)
{
	m_stThemeInfo.nBackDrawType = nDrawType;
}

// 배경 이미지 채우기 방식 설정
void CThemeRadio::SetBackImageFillType(int nImageFillType)
{
	m_stThemeInfo.nBackImageFillType = nImageFillType;
}

// 배경 이미지 정렬방식 설정
void CThemeRadio::SetBackImageAlignFormat(UINT nAlignFormat)
{
	m_stThemeInfo.nBackImageAlignFormat = nAlignFormat;
}

// 툴팁을 생성한다
void CThemeRadio::MakeToolTip(CString strToolTip)
{
	// 현 컨트롤 ID를 구한다
...
// 툴팁 텍스트 변경
void CThemeRadio::ChangeToolTip(CString strToolTip)
{
	// 툴팁 텍스트 변경
...
// 툴팁 삭제
void CThemeRadio::DeleteToolTip()
{
	if( !m_ToolTip.GetSafeHwnd() )
		return;
...
// 툴팁을 위한 이벤트를 받는 함수
LRESULT CThemeRadio::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message)
	{
		case WM_LBUTTONDOWN:
...
// 폰트 컬러 설정
void CThemeRadio::SetFontAndShadowColor(COLORREF crFont, COLORREF crShadow)
{
	// State Normal
...
// 폰트 특성 설정
void CThemeRadio::SetFont(int nFontSize, BOOL bFontBold, COLORREF crFont, COLORREF crShadow, UINT nAlignFormat)
{
	// 폰트 특성을 전역변수에 저장
	m_stThemeInfo.nFontSize = nFontSize;
...
// 배경 컬러 설정
void CThemeRadio::SetBackColor(COLORREF crBackNormal, COLORREF crBackHover, COLORREF crBackPressed, COLORREF crBackDisabled)
{
	m_stThemeInfo.nBackDrawType = dfBACK_DRAW_TYPE_COLOR;		// 배경 그리기 방식
...
void CThemeRadio::CheckHover(CPoint point)
{
...
// 마우스 Hover시 실행되는 함수
LRESULT CThemeRadio::OnMouseHover(WPARAM wParam, LPARAM lParam)
{
...
// 마우스 Leave시 실행되는 함수
LRESULT CThemeRadio::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	// 마우스 Hover 상태로 설정
	if( m_stThemeInfo.bHover )
...
// 마우스 이동시 실행되는 함수
void CThemeRadio::OnMouseMove(UINT nFlags, CPoint point) 
{
	// 마우스 클릭 상태라면
...
// 배경 이미지 블럭 개수 설정
void CThemeRadio::SetBackImageBlockCount(int nBlock)
{
...
CRect CThemeRadio::GetImagePartByState(int nState)
{
...
// 비트맵 이미지로 배경을 그린다
void CThemeRadio::DrawBackByImage(CPaintDC *pdc, CBitmap *pBitmap, CRect rScreen)
{
	int nBtnState=0;
	BOOL bCheck=FALSE;
	...
	CRect rImgPart = GetImagePartByState(nBtnState);
...
// 폰트 컬러 설정
void CThemeRadio::SetFontColor(COLORREF crFontNormal, COLORREF crFontHover, COLORREF crFontPressed, COLORREF crFontDisabled)
{
	// State Normal
...
// 그림자 컬러 설정
void CThemeRadio::SetShadowColor(COLORREF crShadowNormal, COLORREF crShadowHover, COLORREF crShadowPressed, COLORREF crShadowDisabled)
{
	// State Normal
...
// 폰트 Hover 컬러 설정
void CThemeRadio::SetFontHoverColor(COLORREF crFontHover)
{
	m_stThemeInfo.crFont[ST_HOVER] = crFontHover;
}

// 폰트 Pressed 컬러 설정
void CThemeRadio::SetFontPressedColor(COLORREF crFontPressed)
{
	m_stThemeInfo.crFont[ST_PRESSED] = crFontPressed;
}

// 폰트 Disabled 컬러 설정
void CThemeRadio::SetFontDisabledColor(COLORREF crFontDisabled)
{
	m_stThemeInfo.crFont[ST_DISABLED] = crFontDisabled;
}

// 그림자 Hover 컬러 설정
void CThemeRadio::SetShadowHoverColor(COLORREF crShadowHover)
{
	m_stThemeInfo.crShadow[ST_HOVER] = crShadowHover;
}

// 그림자 Pressed 컬러 설정
void CThemeRadio::SetShadowPressedColor(COLORREF crShadowPressed)
{
	m_stThemeInfo.crShadow[ST_PRESSED] = crShadowPressed;
}

// 그림자 Disabled 컬러 설정
void CThemeRadio::SetShadowDisabledColor(COLORREF crShadowDisabled)
{
	m_stThemeInfo.crShadow[ST_DISABLED] = crShadowDisabled;
}

// PushLike 여부 설정
void CThemeRadio::SetPushLike(BOOL bPushLike)
{
	m_stThemeInfo.bPushLike = bPushLike;
}

// 마크 그리기 방식 설정
void CThemeRadio::SetMarkDrawType(int nDrawType)
{
	m_stThemeInfo.nMarkDrawType = nDrawType;

	// 마크를 그리지 않는다면 PushLike로 설정
	if( nDrawType == dfMARK_DRAW_TYPE_NONE )
		m_stThemeInfo.bPushLike = TRUE;
}

// 마크 컬러 설정
void CThemeRadio::SetMarkColor(COLORREF crMark)
{
	m_stThemeInfo.crMark = crMark;
}

// 배경 이미지를 여는 함수(파일)
BOOL CThemeRadio::SetBackImage(CString strFileName)
{
	CString strFilePath;
	BOOL bBmpLoad=FALSE;
...
BOOL CThemeRadio::SetBackImage(UINT nImageID)
{
	BOOL bBmpLoad=FALSE;
...
// 마크 이미지를 여는 함수(파일)
BOOL CThemeRadio::SetMarkImage(CString strFileName)
{
	CString strFilePath;
	BOOL bBmpLoad=FALSE;
	...
	// 이미지 파일을 읽어서 CBitmap으로 반환
	bBmpLoad = gf_LoadPictureFile(strFilePath, &m_stThemeInfo.bmpMark);

	// 이미지 파일 로딩에 성공했다면
	if( bBmpLoad )
	{
		// 마크 그리기 방식을 이미지로 설정
		m_stThemeInfo.nMarkDrawType = dfMARK_DRAW_TYPE_IMAGE;
		
		// 마크 이미지 크기
		BITMAP bm;
		m_stThemeInfo.bmpMark.GetBitmap(&bm);
		m_stThemeInfo.nMarkSize = bm.bmHeight;
	}
	// 이미지 파일 로딩에 실패했다면
	else
	{
		// 마크 그리기 방식을 Default로 설정
		m_stThemeInfo.nMarkDrawType = dfMARK_DRAW_TYPE_COLOR;
	}

	return bBmpLoad;
}

// 마크 이미지를 여는 함수(Resource ID)
BOOL CThemeRadio::SetMarkImage(UINT nImageID)
{
	BOOL bBmpLoad=FALSE;
	...
	// 이미지를 로딩
	bBmpLoad = m_stThemeInfo.bmpMark.LoadBitmap(nImageID);
...
// 마크를 컬러로 그린다
void CThemeRadio::DrawMarkByColor(CPaintDC *pdc, CRect rScreen)
{
	CRect rMark;
	CPoint po1, po2;

	// PushLike 상태라면 마크를 그리지 않는다
	if( m_stThemeInfo.bPushLike )
		return;
	...
	// 화면에 타원(면)을 그린다
	gf_DrawCircle(rMark, m_stThemeInfo.crOutlineLT, 1, RGB(255,255,255), pdc);

	// 현재 Check 상태라면
	if( m_stThemeInfo.bCheck )
	{
		rMark.left += 3;
		rMark.top += 3;
		rMark.right -= 3;
		...
		// 화면에 타원(면)을 그린다
		gf_DrawCircle(rMark, m_stThemeInfo.crOutlineLT, 1, m_stThemeInfo.crMark, pdc);
	}
}

// 마크 영역좌표를 구해서 반환
CRect CThemeRadio::GetMarkArea()
{
	int nMarkSize=12, nLeftGap=0;
	CRect rMark, rScreen;
	...
	// 마크 크기
	nMarkSize = m_stThemeInfo.nMarkSize;

	// 배경을 그린다면
	if( m_stThemeInfo.nBackDrawType != dfBACK_DRAW_TYPE_NONE )
		nLeftGap = 4;

	// 마크 영역좌표 Left,Right 설정
	rMark.left = nLeftGap;
	rMark.right = rMark.left + nMarkSize;

	// 텍스트 정렬방식이 Bottom 이라면
	if( (m_stThemeInfo.nAlignFormat & DT_BOTTOM) == DT_BOTTOM )
	{
		rMark.bottom = rScreen.bottom;
		rMark.top = rMark.bottom - nMarkSize;
	}
	// 텍스트 정렬방식이 VCenter 이라면
	else if( (m_stThemeInfo.nAlignFormat & DT_VCENTER) == DT_VCENTER )
	{
		rMark.top = rScreen.CenterPoint().y - (nMarkSize/2);
		rMark.bottom = rMark.top + nMarkSize;
	}
	// 텍스트 정렬방식이 Top 이라면
	else
	{
...
// 컨트롤 위치좌표를 부모 윈도우에 대한 상대좌표로 반환
CRect CThemeRadio::GetCtrlRectByParent()
{
	CRect rCtrl, rParent;
...
// 비트맵 이미지로 마크를 그린다
void CThemeRadio::DrawMarkByImage(CPaintDC *pdc, CBitmap *pBitmap, CRect rScreen)
{
	CRect rView, rMark;
	...
	// 화면에 표시할 마크 이미지의 부분 영역좌표를 구한다
	rMark.left = 0;
	rMark.right = bm.bmWidth / 2;
	rMark.top = 0;
	rMark.bottom = bm.bmHeight;

	if( m_stThemeInfo.bCheck )
	{
		rMark.left = rMark.right;
		rMark.right = bm.bmWidth;
	}
	
	// 마크를 표시할 화면 영역좌표를 구해서 반환
	rView = GetMarkArea();
	if( rView.Width() != rMark.Width() )
		rView.right = rView.left + rMark.Width();

	// 투명 이미지를 그리는 함수
	gf_DrawTransparentImage(pBitmap, rMark, rView, pdc);
}

void CThemeRadio::PreSubclassWindow() 
{
	// 기본 윈도우 지원 스킨을 방지
	//ModifyStyle(0, BS_OWNERDRAW | BS_AUTORADIOBUTTON);//BS_PUSHBUTTON);
	
	CButton::PreSubclassWindow();
	ModifyStyle(0, BS_OWNERDRAW);
}

void CThemeRadio::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	// 텍스트를 화면에 표시하는 함수
	//DrawText((CPaintDC*)pDC);*/
}

// WM_PAINT 이벤트 발생시 실행되는 함수
void CThemeRadio::OnPaint() 
{
...
// 배경을 지울때 실행되는 함수
BOOL CThemeRadio::OnEraseBkgnd(CDC* pDC) 
{
	// 화면 번쩍거림을 방지
	return true;
...
void CThemeRadio::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CThemeRadio::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// 마우스 클릭 상태 설정
	m_stThemeInfo.bPressed = TRUE;
...
void CThemeRadio::OnLButtonUp(UINT nFlags, CPoint point) 
{
	POINT p2 = point;
...
LRESULT CThemeRadio::OnBMSetCheck(WPARAM wparam, LPARAM)
{
	m_stThemeInfo.bCheck=wparam!=0;
	
	if (m_stThemeInfo.bCheck) { //uncheck the other radio buttons (in the same group)
...
/////////////////////////////////////////////////////////////////////////////
LRESULT CThemeRadio::OnBMGetCheck(WPARAM wparam, LPARAM)
{ return m_stThemeInfo.bCheck; }	//returns the state for check & radio buttons

// 다른 라디오 버튼 컨트롤이 선택되었을때 실행되는 함수
LRESULT CThemeRadio::OnRadioInfo(WPARAM wparam, LPARAM)
{
	if (m_stThemeInfo.bCheck){	//only checked buttons need to be unchecked
		m_stThemeInfo.bCheck = false;
		Invalidate();
	}
	return 0;
}
...
// 툴팁 문자열 설정
void CThemeRadio::SetToolTip(CString strText)
{
	// 툴팁이 이미 생성되어 있다면
	if( m_ToolTip.GetSafeHwnd() )
		// 툴팁 문자열 변경
...
// 윈도우 생성 전에 실행되는 함수
BOOL CThemeRadio::PreCreateWindow(CREATESTRUCT& cs) 
{
	// 'PushLike' 속성이 존재한다면
	if( (cs.style&BS_PUSHLIKE) == BS_PUSHLIKE )
	{
		m_stThemeInfo.bPushLike = TRUE;
	}
...
// 체크 설정/해제
void CThemeRadio::SetCheck(BOOL bCheck)
{
