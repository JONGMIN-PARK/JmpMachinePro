// ThemeStatic.cpp : implementation file
//
...
// 테마 관련 변수 초기화
void CThemeStatic::InitThemeVariable()
{
	...
	m_stThemeInfo.crFont = RGB(8,3,69);							// 폰트 컬러
	...
	m_stThemeInfo.crBackground = RGB(64,192,255);				// 배경 컬러
	m_stThemeInfo.crOutlineLT = RGB(128,128,128);				// 테두리 컬러(Left-Top)
...
BEGIN_MESSAGE_MAP(CThemeStatic, CWnd)
	//{{AFX_MSG_MAP(CThemeStatic)
	ON_WM_PAINT()
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_DESTROY()
...
/////////////////////////////////////////////////////////////////////////////
// CThemeStatic message handlers

// 클래스 생성
BOOL CThemeStatic::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
...
// 컬러로 배경을 그린다
void CThemeStatic::DrawBackByColor(CPaintDC *pdc, COLORREF crBack, CRect rScreen)
{
	// 모서리가 둥글다면
...
// 클래스 생성시 실행되는 함수
int CThemeStatic::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
// Caption 영역좌표 설정
void CThemeStatic::SetCaptionArea(CRect rCaption)
{
	// 전역변수에 저장
...
// Caption 정렬 방식 설정
void CThemeStatic::SetAlignFormat(UINT nAlignFormat)
{
	// 전역변수에 저장
...
// 폰트 컬러 설정
void CThemeStatic::SetFontColor(COLORREF crFont)
{
...
// 그림자 컬러 설정
void CThemeStatic::SetShadowColor(COLORREF crShadow)
{
	m_stThemeInfo.crShadow = crShadow;
}

// 폰트 컬러 설정
void CThemeStatic::SetFontColor(COLORREF crFont, COLORREF crShadow)
{
	m_stThemeInfo.crFont = crFont;
	m_stThemeInfo.crShadow = crShadow;
...
// 폰트 크기 설정
void CThemeStatic::SetFontSize(int nFontSize)
{
	m_stThemeInfo.nFontSize = nFontSize;
}

// 폰트 두께 설정
void CThemeStatic::SetFontBold(BOOL bBold)
{
	m_stThemeInfo.bFontBold = bBold;
}

// 폰트 특성 설정
void CThemeStatic::SetFont(int nFontSize, BOOL bFontBold, COLORREF crFont, COLORREF crShadow, UINT nAlignFormat)
{
	// 폰트 특성을 전역변수에 저장
	m_stThemeInfo.nFontSize = nFontSize;
...
// 배경 이미지를 여는 함수(파일)
BOOL CThemeStatic::SetBackImage(CString strFileName)
{
	CString strFilePath;
	BOOL bBmpLoad=FALSE;
...
BOOL CThemeStatic::SetBackImage(UINT nImageID)
{
...
// 배경 컬러 설정
void CThemeStatic::SetBackColor(COLORREF crBack)
{
...
// 배경 모서리 둥근 정도 설정
void CThemeStatic::SetEdgeRoundValue(int nRoundValue)
{
	m_stThemeInfo.nEdgeRoundValue = nRoundValue;
}

// 외곽선 두께 설정
void CThemeStatic::SetOutlineThick(int nThick)
{
	m_stThemeInfo.nOutlineThick = nThick;
}

// 외곽선(Left-Top) 컬러 설정
void CThemeStatic::SetOutlineLTColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineLT = crOutline;
}

// 외곽선(Right-Bottom) 컬러 설정
void CThemeStatic::SetOutlineRBColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineRB = crOutline;
}

// 외곽선 컬러 설정
void CThemeStatic::SetOutlineColor(COLORREF crOutline)
{
	m_stThemeInfo.crOutlineLT = crOutline;
	m_stThemeInfo.crOutlineRB = crOutline;
}
...
// 배경 그리기 방식 설정
void CThemeStatic::SetBackDrawType(int nDrawType)
{
	m_stThemeInfo.nBackDrawType = nDrawType;
}

// 배경 이미지 채우기 방식 설정
void CThemeStatic::SetBackImageFillType(int nImageFillType)
{
	m_stThemeInfo.nBackImageFillType = nImageFillType;
}

// 배경 이미지 정렬방식 설정
void CThemeStatic::SetBackImageAlignFormat(UINT nAlignFormat)
{
	m_stThemeInfo.nBackImageAlignFormat = nAlignFormat;
}

// 이미지를 Pattern 브러쉬 형식으로 화면 영역에 그리는 함수(바둑판 형식)
void CThemeStatic::DrawImageByPatternBrush(CPaintDC *pdc, CBitmap *pBitmap, CRect rArea)
{
...
// 툴팁을 생성한다
void CThemeStatic::MakeToolTip(CString strToolTip)
{
	// 현 컨트롤 ID를 구한다
...
// 툴팁 텍스트 변경
void CThemeStatic::ChangeToolTip(CString strToolTip)
{
	// 툴팁 텍스트 변경
...
// 툴팁 삭제
void CThemeStatic::DeleteToolTip()
{
...
// 툴팁 문자열 설정
void CThemeStatic::SetToolTip(CString strText)
{
	// 툴팁이 이미 생성되어 있다면
	if( m_ToolTip.GetSafeHwnd() )
		// 툴팁 문자열 변경
...
void CThemeStatic::OnSize(UINT nType, int cx, int cy) 
{
...
LRESULT CThemeStatic::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message)
	{
		case WM_LBUTTONDOWN:
...
// 비트맵 이미지로 배경을 그린다
void CThemeStatic::DrawBackByImage(CPaintDC *pdc, CBitmap *pBitmap, CRect rScreen)
{
	// '배경 이미지 채우기 방식'에 따라서
...
// Caption 문자열 설정
void CThemeStatic::SetWindowText(CString strText)
{
 	SetCaptionText(strText);
}

// Caption 문자열 설정
void CThemeStatic::SetCaptionText(CString strCaption)
{
	// 전역변수에 저장
...
void CThemeStatic::DrawCaptionText(CPaintDC *pdc)
{	
...
// 부모 윈도우를 새로 그린다
void CThemeStatic::RedrawParent()
{
	CRect rCtrl;
	CWnd *pParent;
	...
	pParent->ScreenToClient(rCtrl);

	pParent->InvalidateRect(rCtrl, TRUE);
}